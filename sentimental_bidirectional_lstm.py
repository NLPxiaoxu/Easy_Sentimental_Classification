...
class data_loader(object):
    def __init__(self, data_train):
        ...
        self.in_sentence = np.array(self.in_sentence, np.int32)
        self.in_label = np.array(self.in_label, np.int32)
...
class Sentimental_Singal_Rnn(tf.keras.Model):
    def __init__(self):
        super().__init__()
        self.char_embedding = tf.keras.layers.Embedding(5000, 100, mask_zero=True)
        self.singal_rnn = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(64, return_sequences=True))
...
class Extra_result(object):
    ...
    def call(self):
        ...
        out_score = singal_rnn_model(token)
...
#建立模型
singal_rnn_model = Sentimental_Singal_Rnn()
optimizer = tf.keras.optimizers.Adam(learning_rate=lr)

#保存模型
checkpoint = tf.train.Checkpoint(optimizer=optimizer, singal_rnn_model=singal_rnn_model)
...
for epoch in range(num_epochs):
    ...
    if round(P, 4) > best and round(P, 2) > 0.50:
        ...
        checkpoint.save('./save/bidirectional_lstm_model/version1_checkpoints.ckpt')
        #tf.saved_model.save(singal_rnn_model, './save/bidirectional_lstm_model')
        #singal_rnn_model.save('bidirectional_lstm.h5')
