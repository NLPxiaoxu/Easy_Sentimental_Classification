...
class Sentimental_Bidirectional_Rnn(tf.keras.Model):
    def __init__(self):
        super().__init__()
        self.char_embedding = tf.keras.layers.Embedding(5000, 100, mask_zero=True)
        self.singal_rnn = tf.keras.layers.Bidirectional(tf.keras.layers.SimpleRNN(64, return_sequences=True))
...
class Extra_result(object):
    ...
    def call(self):
        ...
        out_score = bidirectional_rnn_model(token)
...
#建立模型
bidirectional_rnn_model = Sentimental_Bidirectional_Rnn()
optimizer = tf.keras.optimizers.Adam(learning_rate=lr)

#保存模型
checkpoint = tf.train.Checkpoint(optimizer=optimizer, bidirectional_rnn_model=bidirectional_rnn_model)
...
for epoch in range(num_epochs):
    ...
    if round(P, 4) > best and round(P, 2) > 0.50:
        ...
        checkpoint.save('./save/bidirectional_rnn_model/version1_checkpoints.ckpt')